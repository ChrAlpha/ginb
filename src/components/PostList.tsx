...
export const PostCard = memo(
  async ({ title, created_at, summary, path, tags, html_url, comments }: PostCardProps) => {
    return (
...
export const PageNav = memo(async ({ current, prev, prev_link, next, next_link, total }: PageNavProps) => {
  const showPrev = prev && prev != 0;
  const showNext = next && next != 0 && next <= total;

  const linkClassName = "flex flex-row gap-2 rounded-lg border border-transparent hover:border-current p-2 text-black/60 hover:text-black dark:text-white/60 dark:hover:text-white";
  const hiddenClassName = "flex flex-row gap-2 rounded-lg p-2 invisible";
  ...
  return (
    <>
      <div className="mt-4 flex flex-row justify-between">
        {showPrev
          ? (
              <Link className={linkClassName} href={prev_link}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 20 20"
                  fill="currentColor"
        ...
        {showNext
          ? (
              <Link className={linkClassName} href={next_link}>
                <span>Next</span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 20 20"
